...
class inference:
    ...
    def __init__(self, pred_config):
        ...
        self.args = self.get_args(pred_config)
        self.device = self.get_device(pred_config)
        self.model = self.load_model(pred_config, self.args, self.device)
    ...
    def load_model(self, pred_config, args, device):
    ...
    def predict(self, utterance):
        #lines = self.read_input_file(pred_config)
        lines = [utterance.split()]
        dataset = self.convert_input_file_to_tensor_dataset(lines, self.args, self.tokenizer, self.pad_token_label_id)
        ...
        with open(self.pred_config.output_file, "w", encoding="utf-8") as f:
        ...
        return self.intent_label_lst[intent_pred]
